...
{
    /// <summary>Gets or sets a filter which checks if a type is a valid view-model type.</summary>
    /// <value>A filter which checks if a type is a valid view-model type.</value>
    public static Predicate<Type> IsViewModelType { get; set; } = type =>
        ...
        type.Name.EndsWith(value: "ViewModel", StringComparison.InvariantCulture);

    /// <summary>Gets or sets a filter which checks if a type is a valid view type.</summary>
    /// <value>A filter which checks if a type is a valid view type.</value>
    public static Predicate<Type> IsViewType { get; set; } = type =>
        ...
        type.Name.EndsWith(value: "View", StringComparison.InvariantCulture);
