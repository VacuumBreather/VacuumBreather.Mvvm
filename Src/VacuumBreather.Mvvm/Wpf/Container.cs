...
namespace VacuumBreather.Mvvm.Wpf
{
    ...
    public class Container : IServiceProvider
    {
        ...
        public TService[] GetAllInstances<TService>()
        {
            ...
            TService[] instances = this.entries.Where(x => x.Service == service)
        ...
        public object[] GetAllInstances(Type service)
        {
            ...
            object[] instances = this.entries.Where(x => x.Service == service)
        ...
        public object? GetInstance(Type service, string? key = null)
        {
            ...
            ContainerEntry? entry = this.entries.Find(x => x.Service == service && x.Key == key) ??
            ...
            switch (service.IsGenericType)
            {
                case true when DelegateType.IsAssignableFrom(service):
                    {
                        Type typeToCreate = service.GenericTypeArguments[0];
                        ...
                        MethodInfo? factoryFactoryMethod = factoryFactoryType.GetRuntimeMethod(
                            "Create",
                ...
                case true when EnumerableType.IsAssignableFrom(service):
                    {
                        if (key is not null)
                        {
                            throw new InvalidOperationException(
                        ...
                        Type listType = service.GenericTypeArguments[0];
                        object[] instances = GetAllInstances(listType);
                        Array array = Array.CreateInstance(listType, instances.Length);
        ...
        public void RegisterPerRequest(Type service, Type implementation, string? key = null)
        {
            ...
            GetOrCreateEntry(service, key)
                .Add(c => c.BuildInstance(implementation) ?? throw new InvalidOperationException());
        }
        ...
        public void RegisterSingleton(Type service, Type implementation, string? key = null)
        {
            ...
            GetOrCreateEntry(service, key).Add(container =>
                (singleton ??= container.BuildInstance(implementation)) ?? throw new InvalidOperationException());
        }
        ...
        public bool UnregisterHandler(Type service, string? key = null)
        {
            ...
            ContainerEntry? entry = this.entries.Find(
        ...
        protected object? BuildInstance(Type type)
        {
            ...
            object?[] args = constructor.GetParameters().Select(info => GetInstance(info.ParameterType)).ToArray();
        ...
        {
            public static Func<T?> Create(Container container, string key)
            {
